...
class KenaikanKelasImport implements ToCollection, WithChunkReading, SkipsEmptyRows
{
...
public function collection(Collection $rows): void
{
    ...
    if (in_array($status, ['lulus', 'keluar'])) {
        foreach ($rows as $index => $row) {
            ...
            $siswa = \App\Models\Mapping\Siswa::where('nis', $nis)->first();
            if (!$siswa) {
            ...
            if (!$this->dryRun) {
                $siswa->update([
                    'status'      => $status,
                    'tahun_lulus' => \App\Support\TahunAjaran::sekarang(),
                ]);
            }
            $this->lulusCount++;
        }
    ...
    public function getLulusCount(): int     { return $this->lulusCount; }
    public function getSkipped(): array      { return $this->skipped; }
    public function getErrors(): array       { return $this->errors; }
    public function getPreview(): array      { return $this->preview; }
