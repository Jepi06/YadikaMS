...
<body>
    <div class="d-flex">
        <nav class="sidebar p-3" style="width: 260px; flex-shrink: 0;">
            ...
            @foreach ($sidebarJurusan as $j)
                <div class="d-flex justify-content-between align-items-center">
                    <a href="{{ route('spmb.admin.pendaftar.per-jurusan', $j) }}"
                        class="flex-grow-1 d-flex justify-content-between align-items-center {{ request()->routeIs('spmb.admin.pendaftar.per-jurusan') && request()->route('jurusan')?->id === $j->id ? 'active' : '' }}">
                        <span><i class="bi bi-diagram-3 me-2"></i>{{ $j->nama }}</span>
                        <span class="badge bg-secondary badge-kuota me-2">{{ $j->pendaftar_count }}</span>
                    </a>
                    <a href="{{ route('spmb.admin.export.excel.per-jurusan', $j) }}" class="px-2"
                        title="Export Excel {{ $j->nama }}">
            ...
            <a href="{{ route('spmb.admin.export.excel') }}"><i class="bi bi-file-earmark-excel me-2"></i> Export
                Excel</a>
